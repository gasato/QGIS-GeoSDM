...
toDo:
Check input values be progressive
...
class fuzzyMember(QgsProcessingAlgorithm):
    ...
    INPUT_DOUBLE_3 = 'Input_Third'
    INPUT_DOUBLE_4 = 'Input_Fourth'
    ...
    def displayName(self):
        ...
        """
        return self.tr('Fuzzy Membership Trapezoid Functions')
    ...
    def shortHelpString(self):
        ...
        """
        return self.tr("Calculation of degree of membership by different trapezoid membership functions")
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(
            QgsProcessingParameterEnum(
                ...
                ['Linear + -', 'Linear - +', 'Sinusoidal + -', 'Sinusoidal - +']
            )
        )
        ...
        self.addParameter(
            QgsProcessingParameterNumber(
                self.INPUT_DOUBLE_2, 
                self.tr("Input Second Threshold"), 
                QgsProcessingParameterNumber.Double
            )
        )
        ...
        self.addParameter(
            QgsProcessingParameterNumber(
                self.INPUT_DOUBLE_4, 
                self.tr("Input Fourth Threshold"), 
                QgsProcessingParameterNumber.Double
            )
        )
        ...
        self.addParameter(
            QgsProcessingParameterRasterDestination(
                self.OUTPUT,
                self.tr('Fuzzy Membership Function')
            )
        )
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        thirdVal = self.parameterAsDouble(
            parameters,
            self.INPUT_DOUBLE_3,
            context
        )
        
        fourthVal = self.parameterAsDouble(
            parameters,
            self.INPUT_DOUBLE_4,
            context
        )
        ...
        trapezoidUp   = '((r1@1 > %s) AND (r1@1 < %s)) * 1 ' % (secondVal, thirdVal)
        trapezoidDown = '((r1@1 > %s) AND (r1@1 < %s)) * 0 ' % (secondVal, thirdVal)
        
        oneOne   = '(r1@1 < %s) * 1 + (r1@1 > %s) * 1' % (firstVal, fourthVal)
        ceroCero = '(r1@1 < %s) * 0 + (r1@1 > %s) * 0' % (firstVal, fourthVal)
        ...
        if mFunction == 0:
            ...
            l_minus = '((r1@1 >= %s) AND (r1@1 <= %s)) * ((%s - r1@1 ) / (%s - %s))' % (thirdVal, fourthVal, fourthVal, fourthVal, thirdVal)
            formula = ceroCero + ' + ' + l_plus + ' + ' + trapezoidUp + ' + ' + l_minus
        
        elif mFunction == 1:
            l_minus = '((r1@1 >= %s) AND (r1@1 <= %s)) * ((%s - r1@1 ) / (%s - %s))' % (firstVal, secondVal, secondVal, secondVal, firstVal)
            l_plus  = '((r1@1 >= %s) AND (r1@1 <= %s)) * ((r1@1 -  %s) / (%s - %s))' % (thirdVal, fourthVal, thirdVal, fourthVal, thirdVal)
            formula = oneOne + ' + ' + l_minus + ' + ' + trapezoidDown + ' + ' + l_plus
            
        elif mFunction == 2:
            cos_plus = '((r1@1 >= %s) AND (r1@1 <= %s)) * ( 0.5 * ( 1 - cos( 3.141592 * (( r1@1 - %s ) / (%s - %s) ))))' % (firstVal, secondVal, firstVal, secondVal, firstVal)
            cos_minus= '((r1@1 >= %s) AND (r1@1 <= %s)) * ( 0.5 * ( 1 + cos( 3.141592 * (( r1@1 - %s ) / (%s - %s) ))))' % (thirdVal, fourthVal, thirdVal, fourthVal, thirdVal)
            formula =  ceroCero + ' + '  + trapezoidUp + ' + ' + cos_plus + ' + ' + cos_minus
                        
        elif mFunction == 3:
            cos_minus= '((r1@1 >= %s) AND (r1@1 <= %s)) * ( 0.5 * ( 1 + cos( 3.141592 * (( r1@1 - %s ) / (%s - %s) ))))' % (firstVal, secondVal, firstVal, secondVal, firstVal)            
            cos_plus = '((r1@1 >= %s) AND (r1@1 <= %s)) * ( 0.5 * ( 1 - cos( 3.141592 * (( r1@1 - %s ) / (%s - %s) ))))' % (thirdVal, fourthVal, thirdVal, fourthVal, thirdVal)
            formula = oneOne + ' + ' + trapezoidDown + ' + ' + cos_minus + ' + ' + cos_plus
